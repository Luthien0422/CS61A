    ...
    if len(typed_words)==0:
        ...
        while(i<len(reference_words) and i<len(typed_words)):
            if typed_words[i] == reference_words[i]:
                right += 1
            i += 1
        return right * 100.0/len(typed_words) 
    # END PROBLEM 3
...
def autocorrect(user_word, valid_words, diff_function, limit):
    ...
    if user_word in valid_words:
        ...
        small = diff_function(user_word,valid_words[0],limit)
        while i<len(valid_words):
            value = diff_function(user_word,valid_words[i],limit)
            if value < small:
                small = value
                number = i
            i += 1
        if small<=limit:
            return valid_words[number]
        else:
            return user_word
    # END PROBLEM 5
...
def report_progress(typed, prompt, id, send):
    ...
    while(i<len(typed)):
        if typed[i] == prompt[i]:
            ...
            break
    progress = number * 1.0 /len(prompt)
    diction = {'id':id,'progress':progress}
    send(diction)
    return progress
    # END PROBLEM 8
