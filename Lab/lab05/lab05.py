...
def closer_city(lat, lon, city1, city2):
    ...
    'Bucharest'
    """
    distance1 = sqrt((lat - get_lat(city1))**2 + (lon - get_lon(city1))**2)
...
def nut_finder(t):
    ...
    if is_leaf(t) == False:
        if label(t) == 'nut':
            ...
            for trees in branches(t):
                flag = (flag or nut_finder(trees))
            return flag
    else:
        if label(t) == 'nut':
            return True
...
def add_chars(w1, w2):
    ...
    True
    """
    if w1[0] == w2[0] and len(w1)>1 and len(w2)>1:
        return add_chars(w1[1:],w2[1:])
    elif w1[0] == w2[0] and len(w1) == 1 and len(w2)>1:
        return w2[1:]
    elif w1[0] == w2[0] and len(w1) == 1 and len(w2)==1:
        return ''
    else:
